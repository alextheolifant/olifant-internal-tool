...
describe('deriveExpectedClicksPerOrder', () => {
  it('prefers the ad group — the closest comparable population to a search term', () => {
    const r = deriveExpectedClicksPerOrder(
      { clicks: 200, orders: 20 },
      { clicks: 5000, orders: 250 },
      T,
    );
  ...
  it('never divides by zero orders', () => {
    expect(
      deriveExpectedClicksPerOrder({ clicks: 5000, orders: 0 }, null, T),
    ).toBeNull();
  });
  ...
  it('makes the trigger relative: the same clicks judged against different populations', () => {
    ...
    const fast = deriveExpectedClicksPerOrder(
      { clicks: 100, orders: 20 },
      null,
      T,
    )!; // 5
    const slow = deriveExpectedClicksPerOrder(
      { clicks: 400, orders: 10 },
      null,
      T,
    )!; // 40
