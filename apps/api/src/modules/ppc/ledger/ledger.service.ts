...
import { EntityDiffService, valuesMatch } from '../entity-diff/entity-diff.service';
import type { TaskAction } from '../tasks/task.types';
import { LedgerRepository, type LedgerEntryRow, type TaskRow } from './ledger.repository';
...
const ENTITY_TYPES = ['campaign', 'ad_group', 'keyword', 'product_target', 'negative', 'product_ad', 'portfolio'] as const;
...
export class LedgerService {
  ...
  async recordEngineChange(task: TaskRow): Promise<void> {
    ...
    await this.repo.insert({
      ...
      newValue: task.confirmedValue ?? (action.newValue !== null ? String(action.newValue) : null),
      source: 'engine',
      taskId: task.id,
      actor: task.assignee,
      note: null,
  ...
  async detectExternalChanges(accountId: string, fromDate: string, toDate: string): Promise<DetectExternalChangesResult> {
    const context = await this.repo.getAccountContext(accountId);
    if (!context) {
      throw new Error(`detectExternalChanges: no amazon_ads_account found for id ${accountId}`);
    }
    ...
    for (const entityType of ENTITY_TYPES) {
      const diffs = await this.diff.diffAccountState(accountId, entityType, fromDate, toDate);
      for (const d of diffs) {
        for (const change of d.changes) {
          const matchedTask = await this.findMatchingTask(clientId, d.entityType, d.entityId, change.field, change.newValue, toDate);
          if (matchedTask) {
            matched++;
            continue;
          }
    ...
    for (const entry of pending) {
      const campaignName = await this.resolveCampaignName(accountId, entry.entityType, entry.entityId, entry.parentId, toDate);
      await this.repo.insert({
        clientId,
        profile,
        timestampDetected,
  ...
  async listForClient(clientId: string, limit?: number): Promise<LedgerEntryRow[]> {
  ...
  private async findMatchingTask(
    ...
    const candidates = await this.repo.findMatchCandidateTasks(clientId, entityType, entityId);
    ...
    for (const task of candidates) {
      ...
      const expected = task.confirmedValue ?? (action.newValue !== null ? String(action.newValue) : null);
  ...
  private async resolveCampaignName(
    ...
    if (entityType === 'campaign') {
      const row = await this.diff.getSnapshot(accountId, 'campaign', entityId, toDate);
      return nameFromState(row?.state);
    }

    if (entityType === 'ad_group') {
      if (!parentId) return null;
      const row = await this.diff.getSnapshot(accountId, 'campaign', parentId, toDate);
      return nameFromState(row?.state);
    }

    // keyword / product_target / negative / product_ad
    if (!parentId) return null;
    const adGroupRow = await this.diff.getSnapshot(accountId, 'ad_group', parentId, toDate);
    const campaignId = (adGroupRow?.state as { campaignId?: string } | undefined)?.campaignId;
    if (!campaignId) return null;
    const campaignRow = await this.diff.getSnapshot(accountId, 'campaign', campaignId, toDate);
...
export function inferBulkCategories(entries: PendingExternalEntry[]): Map<string, LedgerCategory> {
  ...
  for (const [key, entityIds] of counts) {
    if (entityIds.size >= BULK_OPERATION_THRESHOLD) result.set(key, 'bulk_operation');
  }
  return result;
}
