// Package tokencrypto decrypts ads_manager_accounts.refresh_token, which
// apps/api (NestJS, common/crypto.util.ts) encrypts with AES-256-GCM using
// the same SP_TOKEN_ENCRYPTION_KEY (the same key/algorithm also used for
// amazon_sp_accounts.refresh_token in services/sync-sp-api — this package is
// a verbatim duplicate of that one, since Go's internal/ import-path
// visibility does not allow sharing it directly across the two services).
// Ciphertext layout: base64(iv[12] || authTag[16] || ciphertext) — Go's
// cipher.AEAD.Seal/Open expect the tag appended to the END of the
// ciphertext, so bytes are reordered accordingly on both encode and decode
// to stay wire-compatible with the Node side.
package tokencrypto
